...
const RandomPickManager: React.FC<RandomPickManagerProps> = ({
  ...
  const headerRowClass = isRetroCartoon
    ? 'random-pick-modal__head flex items-center justify-between pb-4'
    : 'flex items-center justify-between border-b border-[rgba(var(--color-accent-primary-rgb),0.2)] pb-4'
    ...
    <div data-theme={theme} className="fixed inset-0 z-[90]">
      ...
      {isDesktop ? (
        <div className="fixed inset-0 z-[91] overflow-y-auto p-4 md:flex md:items-start md:justify-center md:pt-6 md:pb-6">
          <div
            className={`mx-auto my-0 grid w-full max-w-lg gap-4 p-6 max-h-[calc(100vh-3rem)] overflow-y-auto md:my-0 md:max-h-[calc(100vh-3rem)] ${panelClass}`}
          >
            <div className={headerRowClass}>
              <div className="flex items-center gap-3">
                <Shuffle
                  className={`h-5 w-5 ${isRetroCartoon ? 'text-[var(--color-text-primary)]' : 'text-[var(--color-accent-primary)]'}`}
                />
              ...
              <button
                type="button"
                onClick={() => onOpenChange(false)}
                className={isRetroCartoon ? closeButtonClass : `p-2 transition ${closeButtonClass}`}
              >
                <X className="h-5 w-5" />
              </button>
            </div>
              ...
              <div className={`flex ${loadingHeightClass} flex-col items-center justify-center gap-4 p-8 text-center`}>
                ...
                <p
                  className={`text-xs font-bold uppercase tracking-widest text-[var(--color-text-muted)] ${
                    isRetroCartoon ? 'theme-heading-font' : 'font-mono'
                  }`}
                >
                  {items.length > 0
        ...
        <div className="fixed inset-x-0 bottom-0 z-[91]">
          <div
            className={`flex h-auto flex-col pb-10 outline-none ${
              isRetroCartoon
                ? 'random-pick-modal random-pick-modal--sheet rounded-t-3xl border-b-0'
                : isTerminal
                  ? 'rounded-t-none border-t border-[rgba(var(--color-accent-primary-rgb),0.85)] bg-[var(--color-bg-base)]'
                  : isCyberpunk
                    ? 'cyberpunk-surface rounded-t-3xl'
                    : 'rounded-t-3xl border-t border-[rgba(var(--color-accent-primary-rgb),0.3)] bg-black/95'
            }`}
          >
            <div
              className={`mx-auto mt-4 h-1.5 w-12 flex-shrink-0 ${
                isRetroCartoon ? 'rounded-full bg-[var(--color-text-primary)] opacity-35' : isTerminal ? 'bg-[var(--color-accent-primary)]' : 'rounded-full bg-white/20'
              }`}
            />
            ...
            <div className="px-6 pt-6">
              <div
                className={`mb-6 flex items-center justify-between gap-3 ${
                  isRetroCartoon ? 'random-pick-modal__head random-pick-modal__head--drawer' : ''
                }`}
              >
                <div className="flex items-center gap-3">
                  <Shuffle
