...
import { useMediaQuery } from '@/features/shared'
...
const RandomPickManager: React.FC<RandomPickManagerProps> = ({
  ...
  const overlayClass = 'fixed inset-0 z-50 bg-black/80 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0'
          ...
          <Dialog.Content className="fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border border-[rgba(var(--color-accent-primary-rgb),0.3)] bg-black/95 p-6 shadow-2xl duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] rounded-2xl">
            <div className="flex items-center justify-between border-b border-[rgba(var(--color-accent-primary-rgb),0.2)] pb-4">
              <div className="flex items-center gap-3">
                <Shuffle className="h-5 w-5 text-[var(--color-accent-primary)]" />
                <h2 className="text-xl font-black uppercase tracking-tight text-white">{titleText}</h2>
              </div>
              <Dialog.Close className="rounded-full p-2 text-white/50 transition hover:bg-white/10 hover:text-white">
                <X className="h-5 w-5" />
              </Dialog.Close>
            </div>
  ...
  return (
    <Drawer.Root open={isOpen} onOpenChange={onOpenChange}>
      <Drawer.Portal>
        ...
        <Drawer.Content className="fixed inset-x-0 bottom-0 z-50 mt-24 flex h-auto flex-col rounded-t-3xl border-t border-[rgba(var(--color-accent-primary-rgb),0.3)] bg-black/95 pb-10 outline-none">
          <div className="mx-auto mt-4 h-1.5 w-12 flex-shrink-0 rounded-full bg-white/20" />
          
          <div className="px-6 pt-6">
            <div className="mb-6 flex items-center gap-3">
              <Shuffle className="h-5 w-5 text-[var(--color-accent-primary)]" />
              <Drawer.Title className="text-xl font-black uppercase tracking-tight text-white">
                {titleText}
              </Drawer.Title>
            </div>
