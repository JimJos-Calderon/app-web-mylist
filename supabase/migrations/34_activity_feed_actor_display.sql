-- actor_name en activity_feed_view: si user_profiles.username falta, usar prefijo de email
-- desde auth.users (solo lectura vía SECURITY DEFINER; la vista no puede unir auth.users directamente).

BEGIN;

CREATE OR REPLACE FUNCTION public.activity_actor_email_local_part(p_user_id uuid)
RETURNS text
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT NULLIF(split_part(COALESCE(au.email::text, ''), '@', 1), '')
  FROM auth.users au
  WHERE au.id = p_user_id;
$$;

ALTER FUNCTION public.activity_actor_email_local_part(uuid) OWNER TO postgres;

REVOKE ALL ON FUNCTION public.activity_actor_email_local_part(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.activity_actor_email_local_part(uuid) TO authenticated;

COMMENT ON FUNCTION public.activity_actor_email_local_part(uuid) IS
  'Prefijo local del email del usuario (fallback para actor_name en activity_feed).';
...
SELECT
  ...
  COALESCE(
    ...
    public.activity_actor_email_local_part(al.user_id),
    'Unknown User'
  ) AS actor_name,
